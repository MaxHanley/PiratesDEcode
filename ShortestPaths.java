...
public class ShortestPaths {
    ...
    private HashMap<Edge, Integer> weights;	 //  max: gives you a weight when given an edge
    ...
    public void run() {
    	...
    	for( Edge edge : vertex.edgesFrom()) {
    		
    		Decreaser<VertexAndDist> adjVert = handles.get(edge.to);
    		VertexAndDist u = adjVert.getValue();
    		
    		
    		if( v.distance + weights.get(edge) < u.distance) {
    			
    		adjVert.decrease(new VertexAndDist(u.vertex, v.distance + weights.get(edge))); 
    		parentEdges.put(u.vertex,edge);
    		}
    ...
    // edge on a shortest path fromthe starting vertex (stored) to v.
    ...
    public LinkedList<Edge> returnPath(Vertex endVertex) {
    	...
    	path.push(parentEdges.get(endVertex));
    	Vertex v = parentEdges.get(endVertex).from;
    	
    	while(v != startVertex) {
    		
    		path.push(parentEdges.get(v));
